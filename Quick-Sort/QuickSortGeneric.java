...
public class QuickSortGeneric<T>{
    ...
    public static void quicksort(T[]L){
	quicksort(L,0,L.length-1);
    }

    public static void quicksort(T[]L,int left,int right){
    ...
    public static void swap(T[] L, int i, int index){
	T temp=L[i];
	L[i]=L[index];
	L[index]=temp;
    }

    public static int partition(T[] L, int left, int right){

    	int frontIndex=left,
	    temp=0,
	    pivotInitialIndex=left+rand.nextInt(right-left);
	T pivotNum=L[pivotInitialIndex];

	//System.out.println("Pivot number: "+pivotNum);

    	//move the numbers smaller than pivotNum to the front
    	for(int i=left;i<=right;i++){
    	    if(L[i].compareTo(pivotNum)<0){ //(L[i]<pivotNum){
		swap(L,frontIndex,i);
    	    	frontIndex++;
    	    }
